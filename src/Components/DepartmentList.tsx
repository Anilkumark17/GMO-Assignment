...
const DepartmentList: React.FC<{ data: DepartmentData[] }> = ({ data }) => {
  ...
  const handleSelect = (department: string) => {
    ...
    if (isDepartmentSelected) {
      updatedSelected = selected.filter((subDep) => {
        const departmentData = data.find((dep: DepartmentData) => dep.department === department);
        return ![department, ...(departmentData?.sub_departments ?? [])].includes(subDep);
      });
    } else {
      updatedSelected = [...selected, department, ...(data.find((dep: DepartmentData) => dep.department === department)?.sub_departments ?? [])];
    }
  ...
  const handleSubSelect = (subDepartment: string, department: string) => {
    ...
    const allSubsSelected = data
      .find((dep: DepartmentData) => dep.department === department)
      ?.sub_departments.every((subDep: string) => updatedSelected.includes(subDep));
